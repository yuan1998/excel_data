...
class MySelect extends Field
{
     ...
     * @return $this
     */
    public function load($field, $sourceUrl, $currentId, $idField = 'id', $textField = 'text', bool $allowClear = true)
    {
        ...
        $defaultSelect = '"[]"';
        if ($currentId) {
            $projectItem = ProjectType::find($currentId);

            $defaultSelect = $projectItem->archives()->pluck('id');
            $archivesJson  = $projectItem->department
                ->archives()
                ->doesntHave('projects')
                ->orWhereHas('projects', function ($query) use ($currentId) {
...
$(document).on('change', "{$this->getElementClassSelector()}", function () {
    ...
    $.get("$sourceUrl",{q : this.value , id : "$currentId"}, function (data) {
        target.find("option").remove();
        $(target).select2({
            placeholder: $placeholder,
            allowClear: $allowClear,

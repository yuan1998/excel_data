...
class ProjectSelectOfDepartment extends Field
{
     ...
     * @return $this
     */
    public function load($currentId, $field, $idField = 'id', $textField = 'text', bool $allowClear = true)
    {
        ...
        if ($department_id) {
            $defaultSelect = $model->projects()->pluck('id');

            $currentProject = $allProject->filter(function ($item) use ($department_id) {
        ...
        if (currentProject) {
            ...
            var s2 = $(target).select2({
                ...
                data: $.map(currentProject , function (d) {
                    d.id = d.$idField;
                    d.text = d.$textField;
                    return d;
                }),
...
$(document).on('change', "{$this->getElementClassSelector()}", function () {
    ...
    console.log(target,allProject);
    var value = this.value;
    var data = allProject.filter(function (item) {
